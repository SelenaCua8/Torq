...
import { Wrench, ClipboardCheck, LogOut, History, FileText, ChevronRight } from 'lucide-react';
...
export default function MechanicProfile({ auth, mechanicId = 1, mechanicName = 'Operario Técnico' }) {
    ...
    const renderInspections = () => {
        ...
        return items.map((item, idx) => {
            ...
            return (
                <div key={key} className="flex justify-between items-center px-3 py-2.5 rounded-lg transition"
                    style={{ background:'rgba(255,255,255,0.02)', border:'1px solid #1e1e1e' }}>
                    <span className="text-sm max-w-[60%]" style={{ color:'#ccc' }}>{key}. {item.name}</span>
                    <div className="flex items-center gap-2">
                        {obs.trim() !== '' && (
                            <span className="text-[10px] italic px-2 py-0.5 rounded max-w-[140px] truncate" style={{ color:'#666', background:'#111', border:'1px solid #222' }} title={obs}>
                                "{obs}"
                            </span>
                        )}
    ...
    // Calcular horas hasta próximo service
    const horasRestantes = (m) => {
        if (!m.hours_for_service) return null;
        const r = m.hours_for_service - (m.current_hours % m.hours_for_service);
        return r;
    };
            ...
            <main className="flex-1 overflow-y-auto p-8">
                ...
                {!selectedMachine ? (
                    ...
                    <div className="space-y-6 animate-fadeIn">
                        <div>
                            <h1 className="text-3xl font-black tracking-tight">Orden de Trabajo</h1>
                            <p className="text-sm mt-1" style={{ color:'#888' }}>Seleccioná una unidad para iniciar el control o ver su bitácora.</p>
                        </div>

                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
                            {assignedMachines.length === 0 ? (
                                <p className="col-span-full text-center py-12 rounded-2xl text-sm" style={{ color:'#888', background:'#111', border:'1px solid #222' }}>
                                ...
                                return (
                                    <div key={m.id} onClick={() => openMachineTask(m)} className="rounded-2xl flex flex-col cursor-pointer transition-all group"
                                        style={{ background:'#111', border: urgent ? '1px solid rgba(251,191,36,0.4)' : '1px solid #222' }}
                                        onMouseOver={e => e.currentTarget.style.borderColor = urgent ? 'rgba(251,191,36,0.7)' : 'rgba(245,166,35,0.4)'}
                                        onMouseOut={e => e.currentTarget.style.borderColor = urgent ? 'rgba(251,191,36,0.4)' : '#222'}>
                                        ...
                                        <div className="p-5">
                                            ...
                                            <h3 className="font-black text-base mb-1 transition-colors" style={{ color:'#E8E8E8' }}>{m.name}</h3>
                                        ...
                                        <div className="px-5 pb-4 space-y-2">
                                            ...
                                            {hrs !== null && (
                                                <div className="flex justify-between text-xs">
                                                    <span style={{ color:'#888' }}>Próximo service en</span>
                                        ...
                                        <div className="px-5 pb-5">
                                            <div className="w-full py-2.5 rounded-xl text-xs font-bold uppercase tracking-wider flex items-center justify-center gap-2 transition-all"
                                                style={{ background:'rgba(255,255,255,0.03)', color:'#888', border:'1px solid #1e1e1e' }}>
                                                <Wrench className="w-3.5 h-3.5" /> Abrir Ficha de Taller
                    ...
                    <div className="space-y-5 animate-fadeIn flex flex-col h-full">
                        {/* Header */}
                        <div className="flex items-center justify-between p-4 rounded-2xl" style={{ background:'#111', border:'1px solid #222' }}>
                            <div className="flex items-center gap-4">
                                ...
                                <div>
                                    <h2 className="text-xl font-black">{selectedMachine.name}</h2>
                            ...
                            <div className="flex gap-1 p-1 rounded-xl" style={{ background:'#161616', border:'1px solid #222' }}>
                                <button onClick={() => setActiveTab('new')}
                                    ...
                                    <ClipboardCheck className="w-3.5 h-3.5" /> Nuevo Control
                                </button>
                        ...
                        <div className="flex-1 overflow-hidden">
                            {activeTab === 'new' ? (
                                ...
                                <div className="grid grid-cols-1 lg:grid-cols-3 gap-5" style={{ height:'calc(100vh - 220px)' }}>
                                    ...
                                    {/* Lista de planillas */}
                                    <div className="lg:col-span-1 rounded-2xl p-4 flex flex-col overflow-hidden" style={{ background:'#111', border:'1px solid #222' }}>
                                        <h3 className="text-sm font-black pb-3 mb-3" style={{ borderBottom:'1px solid #222' }}>Bitácora</h3>
                                            ...
                                            {maintenanceHistory.length === 0 ? (
                                                ...
                                                <div key={log.id} onClick={() => setSelectedChecklistDetails(log)}
                                                    ...
                                                    <div className="flex justify-between items-center">
                                                        <span className="font-bold" style={selectedChecklistDetails?.id === log.id ? { color: C } : { color:'#E8E8E8' }}>
                                                            Planilla #{log.id}
                                                        </span>
                                                        <span style={{ color:'#666' }}>{new Date(log.created_at).toLocaleDateString()}</span>
                                                    </div>
                                                    <p className="mt-1" style={{ color:'#888' }}>Por: <span style={{ color:'#aaa' }}>{log.mechanic_name}</span></p>
                                                </div>
                                            ))}
                                        </div>
                                    </div>
                                    ...
                                    <div className="lg:col-span-2 rounded-2xl p-5 flex flex-col overflow-hidden" style={{ background:'#111', border:'1px solid #222' }}>
                                        {selectedChecklistDetails ? (
                                            <div className="flex flex-col h-full animate-fadeIn">
                                                <div className="flex justify-between items-center pb-3 mb-3" style={{ borderBottom:'1px solid #222' }}>
                                                    <div>
                                                        <h4 className="font-black text-sm flex items-center gap-2">
                                                            <FileText className="w-4 h-4" style={{ color: C }} /> Detalle de Inspección
                                                        </h4>
                                                        <p className="text-xs mt-0.5" style={{ color:'#888' }}>
                                                            Por {selectedChecklistDetails.mechanic_name}
                                                        </p>
                                                    </div>
                                                ...
                                                <div className="mt-4 p-4 rounded-xl text-xs" style={{ background:'#161616', border:'1px solid #222' }}>
                                                    <strong className="block text-[9px] uppercase tracking-widest mb-1.5" style={{ color: C }}>
                                                        Conclusión del Mecánico:
                                                    </strong>
                                                    <p className="italic leading-relaxed" style={{ color:'#aaa' }}>
                                                        "{selectedChecklistDetails.general_observations || 'Sin comentarios.'}"
                                                    </p>
                                                </div>
                                            </div>
                                        ) : (
                                            <div className="flex-1 flex flex-col items-center justify-center text-center">
                                                <div className="p-4 rounded-full mb-4" style={{ background:'#161616' }}>
                                                    <History className="w-8 h-8" style={{ color:'#333' }} />
                                                </div>
                                                <p className="text-sm max-w-xs" style={{ color:'#888' }}>
                                                    Seleccioná una planilla de la lista para ver el detalle completo.
                                                </p>
                                            </div>
                                        )}
                                    </div>
